/* ============================================================================
 * tarfs.c — TARFS: lendo um arquivo TAR do jeito cru (cap. 12)
 * ----------------------------------------------------------------------------
 * O GRUB carregou o arquivo `initrd.tar` num endereço de memória. Aqui a gente
 * basicamente PARSERIA esse tar na mão, andando de cabeçalho (512 bytes) em
 * cabeçalho. Pra cada arquivo, registramos um nó no VFS com uma função de
 * leitura própria, apontando direto pros dados que já estão em memória.
 *
 * É um sistema "read-only": não tem como escrever num tar. Quem dá conta de
 * escrita é o RAMFS (ramfs.c).
 * ============================================================================ */
...
// Varre o tar carregado na memória, registrando cada arquivo no VFS.
// O `module_address` é onde o GRUB colocou o initrd.tar (já na visão superior).
...
// Procura um arquivo pelo nome e devolve o nó dele (ou 0 se não achar).
